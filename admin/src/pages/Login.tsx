...
export default function Login() {
  ...
  const validate = () => {
    ...
    if (!email.trim()) next.email = 'البريد الإلكتروني مطلوب (Email is required)'
    else if (!/^\S+@\S+\.\S+$/.test(email)) next.email = 'صيغة البريد الإلكتروني غير صحيحة (Invalid email format)'
    if (!password) next.password = 'كلمة المرور مطلوبة (Password is required)'
    else if (password.length < 6) next.password = 'كلمة المرور يجب ألا تقل عن 6 أحرف (Min 6 characters)'
    setErrors(next)
